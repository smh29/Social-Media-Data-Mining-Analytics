 ...
 */
object AmazonReviewRollupSet extends ExecutionApp {
  ...
  def job = Execution.getConfig.flatMap { conf =>
    ...
    val prepared = parsed.map { case (date, score, helpful, uid) =>
      ...
      /*
       * We will count each review once, but also aggregate the set of
       * all users that make reviews so that we can see how many users
       * are active in each of our buckets
       */
      val value = (1L, Set(uid))
      (key, value)
    }
    /*
    ...
    .mapValues { case (count, uniques) =>
      // convert the Set object into a number:
      (count, uniques.size)
    }

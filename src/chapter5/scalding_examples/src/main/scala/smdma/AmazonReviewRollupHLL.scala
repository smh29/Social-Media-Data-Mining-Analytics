 ...
 */
object AmazonReviewRollupHLL extends ExecutionApp {
  ...
  def job = Execution.getConfig.flatMap { conf =>
    ...
    /*
     * HyperLogLog is an approximate technique to do unique counting with
     * much less memory than keeping all of a set and counting at the end
     * We need to use "implicit" so that it can be found when we sumByKey
     * the HLL objects together
     */
    implicit val hll = new HyperLogLogMonoid(conf.get("hll.bits").getOrElse("12").toInt)
    ...
    val prepared = parsed.map { case (date, score, helpful, uid) =>
      ...
      val value = (1L, hll.create(uid.getBytes))
      (key, value)
    }
    /*
    ...
    .mapValues { case (count, approxUniques) =>
      // convert the HLL object into a number:
      (count, approxUniques.estimatedSize)
    }
